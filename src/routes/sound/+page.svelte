...
<div>
  ...
  <div id="sound1">
    <p>
      ...
      simple rhythm.<br> The weird notation below, the <b class="cmd">bd</b> and the
      <b class="cmd">sd</b> are often used to abbreviate drums, as in <b>B</b>ass<b>D</b>rum
      and <b>S</b>nare<b>D</b>rum. </p>
    <p>
      Everything within the quotes <b class="cmd" id="quotes-pattern">" "</b> is interpreted as a pattern. The space
  ...
  <div id="sound2">
    <p>
      2. Others that we can use per default are <b class="cmd">cb</b> and <b class="cmd">hh</b> among
...
{/if}
    ...
    <p id="sound2-3">
      The <b class="cmd">.gain( )</b> command on the end of the line is a useful command. It
      is used to ajust the volume of what is playing. The regular value, the one
      that is on by default, is <b class="cmd">1</b>. If you set the gain to <b class="cmd">0.5</b>,
      that is the half of the volume, and set to <b class="cmd">2</b> we double the volume.
      With the gain set to <b class="cmd">0.0</b> makes it silent.
    </p>
  </div>
  ...
  <div id="sound3">
    <p>
      3. There is also the comma <b class="cmd">(,)</b> which makes sounds play on top of
      eachother at the same time. Notice the difference between grouping with
      a space: <b class="cmd">[hh cb]</b> and grouping with a comma: <b class="cmd">[bd,
      hh]</b>.
    </p>
  </div>
  ...
  <div id="sound4">
    <p>
      ...
      multiplication symbol <b class="cmd">'*'</b>. Combined with a number <b
      class="cmd">'*5'</b> and written close to the name of a sound, it tells
      the sound to play that many times during its normal duration.
    </p>
    <p id="sound4-2">
      To help visualize this, there is a tool called <b
      class="cmd">{".pianoroll()"}</b> that we can use. It makes a graphical
      representation of the sounds, and it is more easy to see what each of the
      different sound are doing. It is special in that you need to write <b
      class="cmd">{".pianoroll({fold: 1})"}</b> to make it work, for reasons we
      won't cover here.
    </p>
  </div>
